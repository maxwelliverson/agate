...
namespace agt {
  ...
  enum class object_type : agt_u16_t {
    ...
    solo_pool_chunk_allocator,
    dual_pool_chunk_allocator,
    private_pool_chunk_allocator,
  ...
  [[nodiscard]] AGT_forceinline thread_state* get_thread_state() noexcept;

  template <typename ObjectType, thread_safety SafetyModel>
  [[nodiscard]] AGT_forceinline ObjectType* alloc(thread_state* state = get_thread_state()) noexcept;
  template <typename ObjectType, thread_safety SafetyModel>
  [[nodiscard]] AGT_forceinline ObjectType* dyn_alloc(size_t size, thread_state* state = get_thread_state()) noexcept;
...
#define AGT_virtual_object_type(objType, ...) struct objType : AGT_concat(PP_AGT_impl_, VIRTUAL_OBJECT_TYPE_##__VA_ARGS__)

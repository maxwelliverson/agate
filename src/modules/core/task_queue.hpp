...
namespace agt {
  ...
  inline static bool remove(locked_task_queue& queue, blocked_task& task) noexcept {
    lock(queue.lock);
  ...
  inline static bool remove(locked_signal_task_queue& queue, blocked_task& task) noexcept {
    lock(queue.lock);

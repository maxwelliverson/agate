...
typedef enum agt_timer_flag_bits_t {
  AGT_TIMER_EXACT_TIMING = 0x1, // Managed by the kernel, the timer going off triggers a signal handler that may interupt agent callbacks and message handling routines.
  AGT_TIMER_
...
AGT_api agt_status_t agt_new_timer(agt_ctx_t ctx, agt_timer_t* pTimer, ) AGT_noexcept;

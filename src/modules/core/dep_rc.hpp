...
namespace agt {
  ...
  namespace impl {
    ...
    AGT_forceinline rc_object*       _aligned(rc_object* obj) noexcept {
      return std::assume_aligned<RcObjectAlignment>(obj);
    }
    AGT_forceinline const rc_object* _aligned(const rc_object* obj) noexcept {
      return std::assume_aligned<RcObjectAlignment>(obj);
    }
    AGT_forceinline rc_object&       _aligned(rc_object& obj) noexcept {
      return *_aligned(&obj);
    ...
    AGT_forceinline bool           _is_user_rc_alloc(const rc_object& obj) noexcept {
    ...
    AGT_forceinline rc_object*     _cast_rc_from_user_address(void* ptr) noexcept {
      AGT_invariant( _is_user_rc_alloc(ptr) );
      return _aligned((rc_object*)(static_cast<char*>(ptr) - offsetof(user_rc_allocation, buffer)));
    }
    AGT_forceinline void*          _cast_user_address_from_rc(rc_object* obj) noexcept {
      AGT_invariant( obj != nullptr );
      AGT_invariant( _is_user_rc_alloc(*obj) );
      return static_cast<user_rc_allocation*>(_aligned(obj))->buffer;
    }

    AGT_forceinline rc_object*     _cast_rc_from_weak_ref(agt_weak_ref_t ref) noexcept {
      return _aligned(reinterpret_cast<rc_object*>(ref));
    }
    AGT_forceinline agt_weak_ref_t _cast_weak_ref_from_rc(rc_object* obj) noexcept {
      return reinterpret_cast<agt_weak_ref_t>(_aligned(obj));
    }

...
namespace agt {
  ...
  template <>
  struct handle_casting<async_data, async_data_t> {
    AGT_forceinline static bool        isa(async_data_t handle) noexcept {
      return (static_cast<uintptr_t>(handle) & 0x1) == 0;
    }
    AGT_forceinline static async_data* cast(async_data_t handle) noexcept {
      return reinterpret_cast<async_data*>(static_cast<uintptr_t>(handle));
    }
  };
